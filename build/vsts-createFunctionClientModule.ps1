...
$parentModule = 'ADMF.Core'
if (-not $ModuleName) { $ModuleName = 'ADMF.Core.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ADMF.Core'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ADMF.Core.psd1"
$prereqHash = @{
