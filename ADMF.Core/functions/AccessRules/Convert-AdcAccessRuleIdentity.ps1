function Convert-AdcAccessRuleIdentity {
	<#
	.SYNOPSIS
	...
	.EXAMPLE
		PS C:\> $adAclObject.Access | Convert-AdcAccessRuleIdentity @parameters
	...
	process {
		:main foreach ($accessRule in $InputObject) {
			...
			if (-not $accessRule.IdentityReference.AccountDomainSid) {
				...
				try { $identity = Get-AdcPrincipal @parameters -Sid $accessRule.IdentityReference -Domain $accessRule.IdentityReference -OutputType NTAccount -Target $Target }
				catch {
					# Empty Catch is OK here, warning happens in command
				}
			}
