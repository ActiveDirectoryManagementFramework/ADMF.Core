function Compare-AdcAccessRules {
	<#
	.SYNOPSIS
	...
	.EXAMPLE
		PS C:\> Compare-AdcAccessRule @parameters -ADRules ($adAclObject.Access | Convert-AdcAccessRuleIdentity @parameters) -ConfiguredRules $desiredPermissions -DefaultRules $defaultPermissions -ADObject $adObject
	...
	$processingMode = Resolve-AdcAccessRuleMode @parameters -ADObject $adObject
	...
	function Write-Result {
		...
		$item = [PSCustomObject]@{
			PSTypeName        = 'Admf.Core.AccessRule.Change'
			Type              = $Type
	...
	$relevantADRules = :outer foreach ($adRule in $ADRules) {
		...
		foreach ($defaultRule in $DefaultRules) {
			if (Test-AdcAccessRuleEquality -Parameters $parameters -Rule1 $adRule -Rule2 $defaultRule) {
	...
	:outer foreach ($relevantADRule in $relevantADRules) {
		foreach ($configuredRule in $ConfiguredRules) {
			if (Test-AdcAccessRuleEquality -Parameters $parameters -Rule1 $relevantADRule -Rule2 $configuredRule) {
	...
	:outer foreach ($configuredRule in $ConfiguredRules) {
		if (-not (Test-Identity -Identity $configuredRule.IdentityReference -Parameters $parameters)) {
			Write-PSFMessage -Level Warning -String 'Compare-AdcAccessRule.Error.BadIdentity' -StringValues $ADObject, $configuredRule.IdentityReference -Target $ADObject
			if (Get-PSFConfigValue -FullName 'ADMF.Core.AccessRule.IdentityErrorChange') {
				Write-Result -Type IdentityError -Identity $configuredRule.IdentityReference -Configuration $configuredRule -DistinguishedName $ADObject
			}
			continue
		}
		...
		foreach ($defaultRule in $DefaultRules) {
			...
			if (Test-AdcAccessRuleEquality -Parameters $parameters -Rule1 $defaultRule -Rule2 $configuredRule) {
