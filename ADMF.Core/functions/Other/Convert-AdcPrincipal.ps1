function Convert-AdcPrincipal {
    ...
    .EXAMPLE
        PS C:\> Convert-AdcPrincipal @parameters -Name contoso\administrator
	...
	process {
		Write-PSFMessage -Level Debug -String 'Convert-AdcPrincipal.Processing' -StringValues $Name
		...
		if ($Name -as [System.Security.Principal.SecurityIdentifier]) {
			Write-PSFMessage -Level Debug -String 'Convert-AdcPrincipal.Processing.InputSID' -StringValues $Name
			...
			$script:cache_PrincipalToNT[$Name] = Get-AdcPrincipal @parameters -Sid $Name -Domain $Name -OutputType NTAccount
			return $script:cache_PrincipalToNT[$Name]
		}
		#endregion Processing Input SID
		
		Write-PSFMessage -Level Debug -String 'Convert-AdcPrincipal.Processing.InputNT' -StringValues $Name
		...
		try {
			...
			$domain = Resolve-Domain @parameters -Name $domainPart -OutputType ADObject
			Write-PSFMessage -Level Debug -String 'Convert-AdcPrincipal.Processing.NTDetails' -StringValues $domainPart, $namePart

			$param = @{
				Server = $domain.DNSRoot
			}
			$cred = Get-AdcDomainCredential -Domain $domain.DNSRoot
			if ($cred) { $param['Credential'] = $cred }
			Write-PSFMessage -Level Debug -String 'Convert-AdcPrincipal.Processing.NT.LdapFilter' -StringValues "(samAccountName=$namePart)"
