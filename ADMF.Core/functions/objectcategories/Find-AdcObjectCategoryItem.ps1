function Find-AdcObjectCategoryItem {
	<#
	.SYNOPSIS
	...
	.EXAMPLE
		PS C:\> Find-AdcObjectCategoryItem -Name 'CAServer'
	
		Find all objects that are part of the CAServer category.
#>
	[Alias('Find-DMObjectCategoryItem')]
	[CmdletBinding()]
	param (
	...
	process {
		...
		if (-not $category) {
			Stop-PSFFunction -String 'Find-AdcObjectCategoryItem.Category.NotFound' -StringValues $Name -EnableException $EnableException -Cmdlet $PSCmdlet
			return
		}
		...
		if ($category.LdapFilter) {
			...
			try { Get-ADObject @parameters -LDAPFilter $filter -SearchBase $searchBase -SearchScope $category.SearchScope -ErrorAction Stop }
				...
				Stop-PSFFunction -String 'Find-AdcObjectCategoryItem.ADError' -StringValues $Name -EnableException $EnableException -Cmdlet $PSCmdlet
				return
			}
		}
		else {
