...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'ADMF.Core.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'ADMF.Core.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'ADMF.Core.Client.ProtectedToken'
